...
function ItemCard({ item }) {
  const { image, ingredients, name, prepTime, price, spicy, type, vegan } = item
  return (
    <ContainerCard className='sm:flex'>
      ...
      <div className='w-full flex flex-col justify-between text-center sm:text-left'>
        ...
        <div className='flex justify-center sm:justify-start text-3xl'>
          <p>{name}</p>
          {spicy && <GiChiliPepper className='ml-2' />}
          {vegan && <SiVectorworks className='ml-2' />}
        </div>
        {/* Description div */}
        <div className='flex flex-col text-base text-slate-100'>
          <p>{ingredients}</p>
          <p>
            Tempo de Preparação: {prepTime}
            {prepTime === '1' ? ' Minuto' : ' Minutos'}
          </p>
        </div>
        {/* Buttons div */}
